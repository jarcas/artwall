...
artwall supports English, German, and Spanish. The tray menu includes **Language → Español / English / Deutsch**; the selected language is saved in `~/.config/artwall/config.json` and reused on future starts. For a first start without an existing configuration, artwall follows `LC_MESSAGES`, `LC_ALL`, or `LANG` when they begin with `en` or `de`; otherwise it uses Spanish.
